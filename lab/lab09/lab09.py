...
class VendingMachine:
    ...
    def vend(self):
        if self.stock <= 0:
            ...
            self.change = self.price - self.balance
            return f'Please update your balance with ${self.change} more funds.'     
        else:
...
def deep_len(lnk):
    ...
    if lnk is Link.empty:
        return 0
    elif isinstance(lnk, int):  # Link.first也是一个链表时，写成elif lnk.rest is Link.empty会导致判断错误
        return 1
    else:
